...
function cost = sidarthe(params1, params2, params3, params4, params5, params6)
    ...
    for i=1:1/step:size(CasiTotali,2)/step
        cost = cost + ( (H_diagnosticati(i)-Guariti(j)) * popolazione )**2;
        j = j + 1;
    end

    j = 1;
    for i=1+3/step:1/step:1+(size(Ricoverati_sintomi,2)+2)/step
        cost = cost + ( (D(i)-Isolamento_domiciliare(j)) * popolazione )**2;
        cost = cost + ( (R(i)-Ricoverati_sintomi(j)) * popolazione )**2;
        cost = cost + ( (T(i)-Terapia_intensiva(j)) * popolazione )**2;
        j = j + 1;
    end
