...
parser.add_argument('--erlang_k',type=int,default=1,help="k parameter for the recovery time Erlang distribution, if set to 1 is an exponential distribution")
...
for seed in range(seed0,seed0+nseed):
    ...
    # -------------------------
    # Initialization
    S,I,R = np.zeros(T_steps),np.zeros([T_steps,erlang_k]),np.zeros(T_steps)
    I[0,0] = I0
    R[0] = R0
    S[0] = N-I0-R0
    I_day[mc_step,0]=I0
    ...
    while (I[t].sum()>0 and day<t_total-1):
        ...
        if(time//day==1):
            ...
            day += 1
        lambda_sum = (delta+beta_func(t)*S[t])*I[t].sum()
        ...
        if (end==True):
            ...
            S[t] = S[t-1]
            I[t] = I[t-1]
            I[t,erlang_k-1] = I[t-1,erlang_k-1] - 1
            R[t] = R[t-1] + 1
        else:
            # infect
            S[t] = S[t-1] - 1
